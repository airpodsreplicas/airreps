...
async function main() {
  ...
  const getAllFiles = (dir) => {
    ...
    list.forEach((file) => {
      ...
      if (stat && stat.isDirectory()) {
        if (!file.includes('node_modules') && !file.includes('.vitepress') && !file.includes('public')) {
  ...
  for (const file of files) {
    ...
    if (!title) continue; // Skip files without title
    ...
    let relPath = path.relative(docsDir, file).replace(/\.md$/, '');
