---
description: "Доверенные ссылки продавцов и варианты покупки реплик AirPods 2."
---

# AirPods 2

#### Версии и продавцы

<table>
  <tr>
    <th>Версия</th>
    <th>Earhive</th>
    <th>HiCity</th>
    <th>Jenny</th>
  </tr>
  <tr>
    <td><span class="version-badge">V3E</span></td>
    <td>
      <div class="link-column">
        <a href="https://airreps.link/e2v3e">Сайт</a>
        <a href="https://airreps.link/earhivew">Weidian</a>
        <a href="https://airreps.link/ehali">Alibaba</a>
      </div>
    </td>
    <td>
      <div class="link-column">
        <a href="https://airreps.link/hcgen2v3e">Сайт</a>
        <a href="https://airreps.link/hicityw">Weidian</a>
      </div>
    </td>
    <td>
      <div class="link-column">
        <a href="https://airreps.link/j2v3e">Сайт</a>
        <a href="https://airreps.link/jennyw">Weidian</a>
      </div>
    </td>
  </tr>
</table>



::: tip
Выбор ссылки Weidian требует использования агента для доставки. Мы рекомендуем эту опцию только если вы хотите объединить доставку с другими товарами и знакомы с агентскими платформами. Для надёжного агента мы рекомендуем [KakoBuy](https://airreps.link/kakobuy). Новички в работе с агентами? Ознакомьтесь с нашим [руководством по заказу KakoBuy](/ordering/kakobuy).
:::
